...
Showdown.converter = ->
  ...
  writeAnchorTag = (wholeMatch, m1, m2, m3, m4, m5, m6, m7) ->
    ...
    if url == ''
      if link_id == ''
        ...
        link_id = link_text.toLowerCase().replace(RegExp(' ?\\n', 'g'), ' ')
      url = '#' + link_id
  ...
  writeImageTag = (wholeMatch, m1, m2, m3, m4, m5, m6, m7) ->
    ...
    if url == ''
      if link_id == ''
        # lower-case and turn embedded newlines into spaces
        link_id = alt_text.toLowerCase().replace(RegExp(' ?\\n', 'g'), ' ')
      url = '#' + link_id
      if g_urls[link_id] != undefined
        url = g_urls[link_id]
        if g_titles[link_id] != undefined
  ...
  _DoHeaders = (text) ->
    ...
    #
    text = text.replace(/^(.+)[ \t]*\n=+[ \t]*\n+/gm, (wholeMatch, m1) ->
      hashBlock '<h1>' + _RunSpanGamut(m1) + '</h1>'
    )
    text = text.replace(/^(.+)[ \t]*\n-+[ \t]*\n+/gm, (matchFound, m1) ->
      hashBlock '<h2>' + _RunSpanGamut(m1) + '</h2>'
    )
    # atx-style headers:
    #  # Header 1
    #  ## Header 2
  ...
  _ProcessListItems = (list_str) ->
    ...
    list_str = list_str.replace(/(\n)?(^[ \t]*)([*+-]|\d+[.])[ \t]+([^\r]+?(\n{1,2}))(?=\n*(~0|\2([*+-]|\d+[.])[ \t]+))/gm, (wholeMatch, m1, m2, m3, m4) ->
      item = m4
      leading_line = m1
      leading_space = m2
      if leading_line or item.search(/\n{2,}/) > -1
  ...
  _DoCodeSpans = (text) ->
    ...
    text = text.replace(/(^|[^\\])(`+)([^\r]*?[^`])\2(?!`)/gm, (wholeMatch, m1, m2, m3, m4) ->
      c = m3
      c = c.replace(/^([ \t]*)/g, '')
      # leading whitespace
      c = c.replace(/[ \t]*$/g, '')
  ...
  _DoAutoLinks = (text) ->
    ...
    text = text.replace(/<(?:mailto:)?([-.\w]+\@[-a-z0-9]+(\.[-a-z0-9]+)*\.[a-z]+)>/gi, (wholeMatch, m1) ->
      _EncodeEmailAddress _UnescapeSpecialChars(m1)
    )
    text
  ...
  _EncodeEmailAddress = (addr) ->
    ...
    addr = addr.replace(/./g, (ch) ->
      if ch == '@'
        ...
        ch = if r > .9 then encode[2](ch) else if r > .45 then encode[1](ch) else encode[0](ch)
      ch
    )
  ...
  _Detab = (text) ->
    ...
    text = text.replace(/~B(.+?)~A/g, (wholeMatch, m1, m2) ->
      leadingText = m1
